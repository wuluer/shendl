...
The most simply implementation of most quick algorithm.
...
/
    ...
    * @return
    * 
> public static int binarySearch(int[.md](.md) array,int start,int end,int value){
...
> > }
> > if(end==(start+1)){
> > > if(array[start](start.md)==value){
> > > > return start;

> > > }else if(array[end](end.md)==value){
> > > > return end;

> > > }


> }else if(array[middle](middle.md)==value){
> > return middle;

> }else if(value>array[middle](middle.md)){
...
/ by 沈东良/良少     http://blog.csdn.net/shendl/
  ...
  * 
> public static void quickSort(int[.md](.md) array,int start,int end){
...
> > > int tmp=array[end](end.md);
> > > array[end](end.md)=array[sign](sign.md);
> > > array[sign](sign.md)=tmp;
> > > int j=start;
> > > for(int i=start;i<=end-1;i++){
> > > > //小于的元素和标记互换，等于的不能互换，否则会形成死循环
> > > > if(array[i](i.md)<array[end](end.md))	{
> > > > > tmp=array[i](i.md);
> > > > > array[i](i.md)=array[j](j.md);
> > > > > array[j](j.md)=tmp;
> > > > > j=j+1;
...
> > > tmp=array[j](j.md);
> > > array[j](j.md)=array[end](end.md);
> > > array[end](end.md)=tmp;
